    ...
    Request (JSON):
        {
            ...
            "graph_id": "mirofish_xxxx",    // Optional; retrieved from the project if not provided
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    # Required files list (excludes scripts, which reside in backend/scripts/)
    required_files = [
        "state.json",
...
def prepare_simulation():
    """
    Prepare the simulation environment (async task; all parameters are generated intelligently by the LLM).
    ...
    Features:
    - Automatically detects completed preparation work to avoid re-generation
    ...
    Request (JSON):
        {
            ...
            "use_llm_for_profiles": true,                 // Optional, whether to generate profiles with LLM
    ...
    try:
        ...
        # Check whether forced re-generation is requested
        force_regenerate = data.get('force_regenerate', False)
        logger.info(f"Processing /prepare request: simulation_id={simulation_id}, force_regenerate={force_regenerate}")

        # Check whether preparation has already been completed (to avoid re-generation)
        if not force_regenerate:
            ...
            if is_prepared:
                logger.info(f"Simulation {simulation_id} is already prepared; skipping re-generation")
                return jsonify({
                    "success": True,
                    "data": {
                        "simulation_id": simulation_id,
                        "status": "ready",
                        "message": "Preparation already complete; no re-generation needed",
        ...
        # This allows the frontend to know the expected total Agent count immediately after calling prepare
        try:
            logger.info(f"Synchronously fetching entity count: graph_id={state.graph_id}")
            reader = ZepEntityReader()
            # Quick read of entities (no edge info needed, just count)
...
def get_prepare_status():
    ...
    2. Check if preparation work has been completed by simulation_id
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "progress": 45,
                "message": "...",
                "already_prepared": true|false,  // Whether preparation has been completed
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    return the most recent one if there are multiple (sorted by created_at).
...
def get_simulation_history():
    ...
    Used for homepage history display, returns simulation list with rich information such as project name and description.
    ...
    try:
        ...
        # Enrich simulation data, read from Simulation files only
        ...
        for sim in simulations:
            ...
            # Get associated project file list (at most 3)
...
def get_simulation_profiles(simulation_id: str):
    """
    Get the simulation Agent Profiles.
...
def get_simulation_profiles_realtime(simulation_id: str):
    ...
    - Returns additional metadata (e.g., file modification time, whether currently generating)
...
def get_simulation_config(simulation_id: str):
    """
    Get simulation config (complete config generated by LLM).
    
    Returns:
        - time_config: Time config (simulation duration, rounds, peak/off-peak periods)
        - agent_configs: Activity config for each Agent (activity level, post frequency, stance, etc.)
...
def start_simulation():
    ...
    About the force parameter:
        ...
        - Does not clear config files (simulation_config.json) and profile files
    ...
    try:
        ...
        if state.status != SimulationStatus.READY:
            ...
            if is_prepared:
                ...
                if state.status == SimulationStatus.RUNNING:
                    ...
                    if run_state and run_state.runner_status.value == "running":
                        ...
                        if force:
                            ...
                            return jsonify({
                                "success": False,
                                "error": f"Simulation is running, please call /stop endpoint to stop it first, or use force=true to force restart"
                            }), 400
                ...
                return jsonify({
                    "success": False,
                    "error": f"Simulation is not ready, current status: {state.status.value}, please call /prepare endpoint first"
                }), 400
...
def interview_agent():
    ...
    Note: This feature requires the simulation environment to be running (enters command-waiting mode after completing the simulation loop).

    Request (JSON):
        {
            "simulation_id": "sim_xxxx",       // Required, simulation ID
            "agent_id": 0,                     // Required, Agent ID
            "prompt": "What is your view on this matter?",  // Required, interview question
    ...
    Returns (platform not specified, dual-platform mode):
        {
            ...
            "data": {
                "agent_id": 0,
                "prompt": "What is your view on this matter?",
                "result": {
                    "agent_id": 0,
                    "prompt": "...",
                    "platforms": {
    ...
    Request (JSON):
        {
            ...
            "platform": "reddit",              // Optional, default platform (overridden per item)
                                               // If not specified: dual-platform each Agent interviews both platforms
    ...
    Request (JSON):
        {
            ...
            "agent_id": 0,                 // Optional, get interview history only for this Agent
...
def close_simulation_env():
    ...
    Send a close environment command to the simulation, causing it to gracefully exit the command-waiting mode.
