...
if sys.platform == 'win32':
    ...
    # Reconfigure standard output streams to UTF-8 (fix garbled console output)
    if hasattr(sys.stdout, 'reconfigure'):
        sys.stdout.reconfigure(encoding='utf-8', errors='replace')
    if hasattr(sys.stderr, 'reconfigure'):
        sys.stderr.reconfigure(encoding='utf-8', errors='replace')
    
    # Force-set the default encoding (affects the default encoding of the open() function)
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None, 
        ...
        """
        Wrapper for the open() function that defaults to UTF-8 encoding for text mode.
        This fixes the issue where third-party libraries (such as OASIS) read files without specifying an encoding.
        """
        # Only set the default encoding for text mode (non-binary) when no encoding is specified
...
def disable_oasis_logging():
    ...
    OASIS logs are too verbose (recording each agent's observations and actions); we use our own action_logger instead.
    """
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Specified platform (optional)
                - "twitter": Interview only on the Twitter platform
                - "reddit": Interview only on the Reddit platform
                - None/not specified: Interview on both platforms simultaneously, return merged result
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Default platform (can be overridden per interview item)
                - "twitter": Interview only on the Twitter platform
                - "reddit": Interview only on the Reddit platform
                - None/not specified: Interview each Agent on both platforms simultaneously
        """
        # Group by platform
        twitter_interviews = []
        reddit_interviews = []
...
# Non-core action types to be filtered out (these actions have lower analytical value)
...
def fetch_new_actions_from_db(
    ...
    Args:
        ...
        last_rowid: The maximum rowid read last time (using rowid instead of created_at because
                    created_at format differs across platforms)
        agent_names: agent_id -> agent_name mapping

    Returns:
        (actions_list, new_last_rowid)
        - actions_list: List of actions; each element contains agent_id, agent_name, action_type,
                        action_args (with context information)
...
def _enrich_action_context(
    ...
    """
    Enrich action context information (post content, username, etc.).
    ...
    try:
        ...
        # Repost: add original post content and author
            ...
            if new_post_id:
                # The original_post_id of the repost points to the original post
                cursor.execute("""
        ...
        # Follow user: add the name of the followed user
        ...
        # Mute user: add the name of the muted user
...
def _get_post_info(
    ...
    try:
        ...
        if row:
            ...
            # Prefer using the name from agent_names
            author_name = ''
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    If a boost LLM is configured, different platforms can use different API providers
    during parallel simulation to improve concurrency.
    ...
    # Select which LLM to use based on arguments and configuration
    ...
    # If no model name is found in .env, fall back to config
    if not llm_model:
...
async def run_twitter_simulation(
    ...
    # If a certain agent is not in the config, use OASIS's default name
    ...
    # Log round 0 start (initial event stage)
    if action_logger:
    ...
    # Truncate if a maximum number of rounds is specified
...
async def run_reddit_simulation(
    ...
    # Reddit uses the boost LLM configuration (falls back to the general config if unavailable)
...
async def main():
    ...
    # Create the shutdown event at the start of main to ensure the entire program can respond to exit signals
        ...
        try:
            while not _shutdown_event.is_set():
                ...
                if not should_continue:
                    break
                # Use wait_for instead of sleep so that shutdown_event can be responded to
                try:
...
def setup_signal_handlers(loop=None):
    """
    Set up signal handlers to ensure correct exit when SIGTERM/SIGINT is received.

    Persistent simulation scenario: do not exit after simulation completes; wait for interview commands.
    When a termination signal is received, the handler must:
    1. Notify the asyncio loop to stop waiting
    2. Give the program a chance to clean up resources properly (close database, environment, etc.)
    3. Then exit
    """
