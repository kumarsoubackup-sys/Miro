    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 0, 'completed': phase > 0 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Creates a new simulation instance and fetches the simulation world parameter template.
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Using context, automatically invokes tools to map entities and relationships from the knowledge graph, initializes simulation individuals, and assigns them unique behaviors and memories based on the reality seed.
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            The LLM intelligently configures world time flow, recommendation algorithms, active time slots, posting frequency, event triggers, and other parameters based on the simulation requirements and reality seed.
          </p>
          ...
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                ...
                <span class="section-desc">MiroFish automatically plans to simulate <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || ‘-’ }}</span> real-world hours, with each round representing <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || ‘-’ }}</span> minutes of real-world time.</span>
              </div>
              ...
              <div v-else class="rounds-content auto" key="auto">
                <div class="auto-info-card">
                  ...
                  <div class="auto-content">
                    ...
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">For first-time runs, it is strongly recommended to switch to ‘Custom Mode’ to reduce rounds for a quick preview and to lower error risk ➝</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div class="modal-section" v-if="selectedProfile.persona">
            ...
            <div class="persona-dimensions">
              ...
              <div class="dimension-card">
                ...
                <span class="dim-desc">Memories formed based on the reality seed</span>
              </div>
              <div class="dimension-card">
...
// Log deduplication: tracks the last key info that was logged
...
watch(currentStage, (newStage) => {
  if (newStage === 'generating_profiles' || newStage === 'generating_profiles') {
    ...
    phase.value = 2
    // Entered config generation stage, start polling for config
    if (!configTimer) {
...
const autoGeneratedRounds = computed(() => {
  if (!simulationConfig.value?.time_config) {
    return null // return null if config is not yet generated
  }
...
// Calculate total number of associated topics for all profiles
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.already_prepared) {
        addLog('Existing completed preparation detected, reusing')
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (currentCount > 0 && currentCount !== lastLoggedProfileCount) {
        ...
        addLog(`→ Agent profile ${currentCount}/${total}: ${profileName} (${latestProfile?.profession || ''})`)
    ...
    if (res.success && res.data) {
      ...
      if (data.generation_stage && data.generation_stage !== lastLoggedConfigStage) {
        ...
        if (data.generation_stage === 'generating_profiles') {
          addLog('Generating agent profile configurations...')
      ...
      if (data.config_generated && data.config) {
        ...
        if (data.summary) {
          ...
          addLog(`  └─ Platform config: Twitter ${data.summary.has_twitter_config ? '\u2713' : '\u2717'}, Reddit ${data.s`)
        }
