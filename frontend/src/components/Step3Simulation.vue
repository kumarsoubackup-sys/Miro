...
// Format simulation elapsed time (calculated from rounds and minutes per round)
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Detect if simulation has completed (via runner_status or platform complete status)
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // Extra check: if backend has not yet updated runner_status but platforms already report complete
...
const checkPlatformsCompleted = (data) => {
  ...
  // Determine if platform is enabled by actions_count (count > 0 or running was true)
  const twitterEnabled = (data.twitter_actions_count > 0) || data.twitter_running || twitterCompleted
  const redditEnabled = (data.reddit_actions_count > 0) || data.reddit_running || redditCompleted
  
  // Return false if no platforms enabled
