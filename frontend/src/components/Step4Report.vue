...
const toggleRawResult = (timestamp, event) => {
  ...
  // Wait for DOM update, then adjust scroll position to keep button in same position
...
const parseInterview = (text) => {
  ...
  try {
    ...
    const reasonMatch = text.match(/### Selection Reasoning\n([\s\S]*?)(?=\n---\n|\n### Interview Transcript)/)
    if (reasonMatch) {
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (answerMatch) {
        ...
        if (!twitterMatch && redditMatch) {
          // Only Reddit answer, copy as default display only if not a placeholder text
          ...
          // No platform markers (very old format), treat full text as answer
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Drift query generates sub-questions for analysis'),
        ...
        // Empty state
        activeTab.value === 'facts' && props.result.facts.length === 0 && h('div', { class: 'empty-state' }, 'No facts found')
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'entities' && h('div', { class: 'entities-panel' }, [
          ...
          ) : h('div', { class: 'empty-state' }, 'No entities found'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Check if it is a platform placeholder text
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // If no match, fall back to "digit." format
      if (matches.length === 0) {
        const numPattern = /(?:^|[\r\n]+)(\d+)\.\s+/g
      ...
      // If no numbered sections found or only one found, return full text
      if (matches.length <= 1) {
    ...
    // Get answer corresponding to a question
...
const activeStep = computed(() => {
  ...
  // If none active, return the last done step
...
const renderMarkdown = (content) => {
  ...
  // Remove leading level-2 heading (## xxx), since section title is displayed in outer container
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            ...
            stopPolling()
            // Scroll logic unified in nextTick after loop ends
          }
...
const extractFinalContent = (response) => {
  ...
  // If content is long and contains markdown format, try removing thinking process before returning
