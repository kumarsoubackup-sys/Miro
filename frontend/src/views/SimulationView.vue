 ...
 * Called when user returns from Step 3 to Step 2 — user intent is to exit simulation.
 */
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      try {
        ...
        if (closeRes.success) {
          ...
          // If graceful close fails, force stop
          await forceStopSimulation()
        }
      } catch (closeErr) {
        addLog(`Error closing simulation environment: ${closeErr.message}`)
        // If graceful close throws, force stop
        await forceStopSimulation()
      }
    } else {
...
const forceStopSimulation = async () => {
  try {
    ...
    addLog(`Error during force stop: ${err.message}`)
  }
}
